...
function NotificationsContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch Channels
      const channelsRes = await fetch("http://localhost:3003/api/notifications/channels", {
      ...
      // Fetch Events Log
      const eventsRes = await fetch("http://localhost:3003/api/notifications/events", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (eventsRes.ok) {
  ...
  const handleAddChannel = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:3003/api/notifications/channels", {
        method: "POST",
        headers: {
  ...
  const handleRemoveChannel = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:3003/api/notifications/channels/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
